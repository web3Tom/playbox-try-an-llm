...
Demonstrates the orchestrator pattern: `gpt-5.4` (reasoning model) reads a specification and produces an implementation plan, which is then ready for delegation to `gpt-5-mini` or a code agent.
...
- **gpt-5-mini**: Implementation from plan
...
3. Prints the plan
4. (Optional next step) Pass plan to gpt-5-mini or Kilo Code for implementation

## Model Routing

This demo illustrates the Polestar principle: **different models for different jobs**.
- Heavy reasoning → `gpt-5.4`
- Implementation from plan → `gpt-5-mini`
- Lightweight tasks → `gpt-5-nano`
