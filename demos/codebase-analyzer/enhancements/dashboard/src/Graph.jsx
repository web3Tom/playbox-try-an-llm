...
// Enhanced graph: files render as COMPOUND nodes (boxes) that contain their
// top-level functions and classes as child nodes. Containment is derived from
// the shared `filePath` — a member is parented to the file node with the same
// path. Import edges connect the file boxes. Colour follows architectural layer;
// shape distinguishes file (box) / function (ellipse) / class (hexagon).
  ...
  useEffect(() => {
    ...
    const elements = [
      ...
      ...(showMembers ? memberNodes : []).map((n) => {
        const parent = pathToFileId[n.filePath]
        return {
          data: { id: n.id, label: n.name, layer: n.layer || 'none', kind: n.type, parent }
        }
      }),
    ...
    const cy = cytoscape({
      ...
      style: [
        ...
        {
          // Functions: small filled ellipse with light label inside.
        ...
        {
          // Classes: hexagon to read differently from functions at a glance.
        ...
        {
          // Any node that has children becomes a labelled container box.
          selector: ':parent',
          style: {
    ...
    if (q) {
      ...
      cy.nodes().forEach((n) => {
        if ((n.data('label') || '').toLowerCase().includes(q)) {
          matched.add(n.id())
          n.parent().forEach((p) => matched.add(p.id())) // keep the box visible
